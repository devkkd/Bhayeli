...
export async function generateMetadata({ params }) {
  ...
  return { title: category ? `${category.title} | Bhayeli` : "Category | Bhayeli" };
}

export default async function CategoryPage({ params }) {
  const [categories, products] = await Promise.all([
    fetchCategories(),
    fetchProducts({ categorySlug: params.slug }),
  ]);

  const category = categories.find((c) => c.slug === params.slug);
  if (!category) notFound();

  const whatsappBase = "https://wa.me/917792811100?text=Hi%2C%20I%27m%20interested%20in%20";
    ...
    <main className="w-full bg-[#FCF9F4] min-h-screen" style={{ fontFamily: "var(--font-philosopher)" }}>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pt-8">
        <nav className="flex items-center gap-2 text-[13px] text-gray-400">
          ...
          <span>/</span>
          <Link href="/categories" className="hover:text-[#1a1a2e] transition-colors">Categories</Link>
          <span>/</span>
          <span className="text-[#1a1a2e] font-medium">{category.title}</span>
        </nav>
      </div>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {categories.map((c) => (
              <Link key={c.slug} href={`/categories/${c.slug}`}
                className={`text-[13px] font-medium px-5 py-2 rounded-full border transition-colors whitespace-nowrap ${c.slug === params.slug
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pb-20">
        {products.length === 0 ? (
          <div className="text-center py-24">
            <p className="text-[16px] text-gray-400">No products in this category yet.</p>
            <p className="text-[14px] text-gray-400 mt-2">Add products from the admin panel.</p>
          </div>
        ) : (
            ...
            {products.map((product) => (
              <div key={product._id} className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-0.5 px-1">
                  <h3 className="text-[#1a1a2e] text-[14px] md:text-[16px] font-semibold leading-snug">{product.title}</h3>
                ...
                <div className="flex items-center gap-2 px-1 flex-wrap">
                  ...
                  <a href={`${whatsappBase}${encodeURIComponent(product.title)}`} target="_blank" rel="noopener noreferrer"
                    className="w-8 h-8 rounded-full bg-[#25D366] flex items-center justify-center hover:bg-[#1ebe5d] transition-colors shrink-0" aria-label="WhatsApp">

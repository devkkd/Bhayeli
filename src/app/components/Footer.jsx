...
export default function Footer() {
  ...
  const techniques = [
    { label: "Block Print",     href: "/techniques#block-print" },
    { label: "Hand Printing",   href: "/techniques#hand-printing" },
    { label: "Embroidery",      href: "/techniques#embroidery" },
    { label: "Digital Print",   href: "/techniques#digital-print" },
  ]; 
  ...
  return (
    <footer className="w-full bg-[#f5f0e8]">
      ...
      <div className="max-w-8xl mx-auto px-6 md:px-12 lg:px-20 py-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-5">
        ...
        <div className="flex flex-col gap-3">
          ...
          <ul className="flex flex-col gap-2">
            {quickLinks.map((item) => (
              <li key={item.href}>
                <Link href={item.href} className="text-[12px] text-gray-500 hover:text-[#1a1a2e] transition-colors">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div className="flex flex-col gap-3">
          ...
          <ul className="flex flex-col gap-2">
            {categories.length > 0 ? (
              categories.map((cat) => (
                <li key={cat._id || cat.slug}>
                  <Link
                    ...
                    className="text-[12px] text-gray-500 hover:text-[#1a1a2e] transition-colors"
                  >
                    {cat.title}
                  </Link>
                </li>
              ...
              [1, 2, 3, 4].map((i) => (
                <li key={i} className="h-3 w-24 bg-gray-200 rounded animate-pulse" />
              ))
            )}
          </ul>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 lg:px-20 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
        ...
        <p className="text-[12px] text-gray-400">Crafted by : <Link href={"https://www.kontentkraftdigital.com"} target="_blank"> Kontent Kraft Digital </Link> </p>
      </div>

...
// Fetch all categories for the filter pills + metadata
...
// Fetch products filtered by collectionSlug
async function getProducts(slug) {
  try {
    const res = await fetch(`${BASE_URL}/api/products?collectionSlug=${slug}`, { cache: "no-store" });
    const data = await res.json();
    return data.success ? data.data : [];
  } catch {
    return [];
...
export async function generateMetadata({ params }) {
  ...
  return { title: category ? `${category.title} | Bhayeli` : "Collection | Bhayeli" };
}
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pb-20">
        {products.length === 0 ? (
          <div className="text-center py-24">
            <p className="text-[16px] text-gray-400">No products in this collection yet.</p>
